...
-- Panel Admin - Listar usuarios con roles
...
/*!50003 SET sql_mode              = 'NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_CambiarRolUsuario`(
    IN pIdUsuario INT,
    IN pNuevoIdRol INT
)
BEGIN
    DECLARE vRolActual INT;
    
    -- Obtener rol actual
    SELECT id_rol INTO vRolActual
    FROM usuarios
    WHERE id_usuario = pIdUsuario
    LIMIT 1;
    
    -- Verificar que el rol nuevo existe
    IF NOT EXISTS (SELECT 1 FROM roles WHERE id_rol = pNuevoIdRol) THEN
        SIGNAL SQLSTATE '45000'
